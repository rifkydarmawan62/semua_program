try:
    ...
    def menu_integrasi_powershell():
        ...
        print("[7] tampilkan perintah (command)")
        ...
        def pilih_menu():
            ...
            match menu_yang_dipilih:
                ...
                case "9":
                    ...
                    if direktori_file:
                        subprocess.run(["powershell", f"format-hex \"{direktori_file}\""], shell = True, text = True)
                    else:
        ...
        def pilih_menu():
            ...
            match menu_yang_dipilih:
                ...
                case _:
                    ...
                    pilih_menu()
        pilih_menu()
    def menu_python_zipfile():
        def enter_untuk_kembali_ke_menu_python_zipfile():
            input("Tekan Enter untuk kembali (Ctrl + C untuk keluar)")
            menu_python_zipfile()
        bersihkan_layar()
        print("beranda > python zipfile\n")
        cetak_informasi_pengembang()
        print("Modul standar Python zipfile untuk mengelola file .zip\n")
        print("Daftar menu Python zipfile")
        print("[-] bersihkan layar")
        print("[-0] beranda")
        print("[-00] keluar")
        print("[#:] buka file .zip")
        ...
        def pilih_menu():
            ...
            match menu_yang_dipilih:
                ...
                case "-00":
                    ...
                    exit(0)
                case "#:":
                    print("Tekan Alt + Tab untuk membuka jendela untuk memilih file zip untuk dibuka")
                    direktori_file_zip = askopenfilename(title = "Buka File Zip", filetypes = [("Arsip Zip", "*.zip")])
                    if direktori_file_zip:
                        print("Memeriksa file zip ...")
                        if zipfile.is_zipfile(direktori_file_zip):
                            print("File .zip valid")
                            menu_opsi_direktori_file_zip(direktori_file_zip)
                        else:
                            print("File Yang Dibuka Bukan File .zip!")
                            enter_untuk_kembali_ke_menu_python_zipfile()
                    else:
                        print("File Zip Tidak Dibuka!")
                        enter_untuk_kembali_ke_menu_python_zipfile()
                case "%":
                    tampilkan_penggunaan_memori()
                    enter_untuk_kembali_ke_menu_python_zipfile()
                case _:
                    ...
                    pilih_menu()
        pilih_menu()
    def menu_opsi_direktori_file_zip(__direktori_file_zip : str):
        def enter_untuk_kembali_ke_menu_opsi_direktori_file_zip(__direktori_file_zip : str):
            input("Tekan Enter untuk kembali (Ctrl + C untuk keluar)")
            menu_opsi_direktori_file_zip(__direktori_file_zip)
        bersihkan_layar()
        print(f"beranda > python zipfile > opsi direktori {__direktori_file_zip}\n")
        print("Menu opsi file zip")
        print("[-] bersihkan layar")
        print("[-0] kembali")
        print("[-00] beranda")
        print("[-000] keluar")
        print("[%] tampilkan penggunaan memori")
        print("[1] tampilkan seluruh direktori file arsip zip")
        print("[2] tampilkan seluruh informasi file arsip zip")
        print("[3] tampilkan seluruh direktori dan informasi file arsip zip")
        print("[4] ekstrak seluruh file arsip zip")
        print("[5] masukkan kata sandi valid lalu ekstrak seluruh file arsip zip")
        def baca_file_zip(__direktori_file_zip : str, __opsi : Literal["tampilkan seluruh direktori file arsip zip", "tampilkan seluruh informasi file arsip zip", "tampilkan seluruh direktori dan informasi file arsip zip", "ekstrak seluruh file arsip zip", "input kata sandi file arsip zip"]):
            def pesan_error_kompresi_tidak_valid():
                print("File arsip zip tidak cocok dengan tipe kompresi file!")
                print("Periksa properti file zip apakah tipe kompresi file cocok atau tidak")
            with zipfile.ZipFile(__direktori_file_zip) as file_zip:
                match __opsi:
                    case "tampilkan seluruh direktori file arsip zip":
                        print("Direktori file arsip zip : ")
                        for direktori_arsip_zip in file_zip.namelist():
                            print(direktori_arsip_zip)
                    case "tampilkan seluruh informasi file arsip zip":
                        print("Informasi file arsip zip : ")
                        for info_zip in file_zip.infolist():
                            print(f"Direktori atau nama file arsip : {info_zip.filename}")
                            print(f"Ukuran file : {info_zip.file_size} byte")
                            print(f"File dibuat {info_zip.date_time}\n")
                    case "tampilkan seluruh direktori dan informasi file ars…":
                        file_zip.printdir()
                    case "ekstrak seluruh file arsip zip":
                        print("Tekan Alt + Tab untuk membuka jendela baru untuk ekstraksi file zip")
                        lokasi_folder = askdirectory(title = "Lokasi Folder Untuk Ekstraksi File Zip")
                        if lokasi_folder:
                            print(f"Mengekstraksi file zip ke folder {lokasi_folder}...")
                            try:
                                file_zip.extractall(path = lokasi_folder)
                            except RuntimeError:
                                print("File arsip zip terkunci kata sandi!")
                                print("Kembali dan coba pilih menu [5, 6, 7] untuk meretas kata sandi file zip")
                            else:
                                print("Ekstraksi selesai!")
                        else:
                            print("Folder Tidak Dibuka!")
                    case _:
                        file_zip.close()
            match __opsi:
                case "input kata sandi file arsip zip":
                    print("Tekan Alt + Tab untuk membuka jendela baru untuk ekstraksi file zip")
                    lokasi_folder = askdirectory(title = "Lokasi Folder Untuk Ekstraksi File Zip")
                    if lokasi_folder:
                        kata_sandi_valid = input("Masukkan kata sandi valid file arsip zip : ")
                        tipe_kompresi = input("Pilih salah satu tipe kompresi file yang didukung [deflated | bzip2 | l2ma | tanpa kompresi] : ")
                        print(f"Memulai ekstraksi file zip ke folder {lokasi_folder}")
                        try:
                            def ekstrak_file_zip(__direktori_file_zip : str, __tipe_kompresi : int):
                                with zipfile.ZipFile(__direktori_file_zip, "r", __tipe_kompresi) as file_zip:
                                    for info_zip in file_zip.infolist():
                                        print(f"Mengekstraksi arsip {info_zip.filename}")
                                        file_zip.extract(path = lokasi_folder, pwd = str.encode(kata_sandi_valid), member = info_zip)
                            match tipe_kompresi:
                                case "deflated":
                                    ekstrak_file_zip(__direktori_file_zip, zipfile.ZIP_DEFLATED)
                                case "bzip2":
                                    ekstrak_file_zip(__direktori_file_zip, zipfile.ZIP_BZIP2)
                                case "l2ma":
                                    ekstrak_file_zip(__direktori_file_zip, zipfile.ZIP_LZMA)
                                case "tanpa kompresi":
                                    ekstrak_file_zip(__direktori_file_zip, zipfile.ZIP_STORED)
                                case _:
                                    print("Input Tipe Kompresi File Tidak Valid!")
                        except NotImplementedError:
                            pesan_error_kompresi_tidak_valid()
                        else:
                            print("Ekstraksi berhasil!")
                            print(f"Kata sandi valid : {kata_sandi_valid}")
                    else:
                        print("Folder Tidak Dibuka!")
        def pilih_menu():
            menu_yang_dipilih = input("Pilih nomor : ")
            match menu_yang_dipilih:
                ...
                case "%":
                    tampilkan_penggunaan_memori()
                    enter_untuk_kembali_ke_menu_opsi_direktori_file_zip(__direktori_file_zip)
                case "1":
                    baca_file_zip(__direktori_file_zip, "tampilkan seluruh direktori file arsip zip")
                    enter_untuk_kembali_ke_menu_opsi_direktori_file_zip(__direktori_file_zip)
                case "2":
                    baca_file_zip(__direktori_file_zip, "tampilkan seluruh informasi file arsip zip")
                    enter_untuk_kembali_ke_menu_opsi_direktori_file_zip(__direktori_file_zip)
                case "3":
                    baca_file_zip(__direktori_file_zip, "tampilkan seluruh direktori dan informasi file ars…")
                    enter_untuk_kembali_ke_menu_opsi_direktori_file_zip(__direktori_file_zip)
                case "4":
                    baca_file_zip(__direktori_file_zip, "ekstrak seluruh file arsip zip")
                    enter_untuk_kembali_ke_menu_opsi_direktori_file_zip(__direktori_file_zip)
                case "5":
                    baca_file_zip(__direktori_file_zip, "input kata sandi file arsip zip")
                    enter_untuk_kembali_ke_menu_opsi_direktori_file_zip(__direktori_file_zip)
                case _:
                    print("Input Tidak Valid!")
                    pilih_menu()
    ...
    def beranda():
        ...
        print("Peringatan : Program ini memiliki bug kebocoran memori yang meningkat seiring pengguna memilih menu baru. Anda dapat menggunakan % untuk menampilkan penggunaan memori\n")
        print("Daftar Menu")
        print("[%] tampilkan penggunaan memori")
        print("[1] kriptografi")
        print("[2] kalkulator")
        ...
        print("[7] python zipfile")
        print("[8] bersihkan")
        print("[9] keluar (Ctrl + C)\n")
        def pilih_menu():
            menu_yang_dipilih = input("Pilih nomor : ")
            match menu_yang_dipilih:
                case "%":
                    tampilkan_penggunaan_memori()
                    input("Tekan Enter untuk reset (Ctrl + C untuk keluar)")
                    beranda()
                case "1":
                    menu_kriptografi()
                case "2":
    ...
    print("Menutup program ...")
    exit(0)
except ModuleNotFoundError as modul_tidak_ada:
    print("Modul tidak valid")
    print("Pastikan anda telah mengunduh seluruh modul dengan perintah git yang benar")
    print(f"Log error :\n{modul_tidak_ada}")
